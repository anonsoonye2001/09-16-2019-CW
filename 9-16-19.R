...
library(openxlsx)
openXL(file=)
d<-read.table(file="import_datasets/Aurelia_SEAMAP_2012-2018_30minCell.csv",
...
head(d1)
d2<-read.csv(file="import_datasets/aurelia_15minCell_statareas.txt",header=T,
             sep=",",stringsAsFactors = F)
head(d2)
d3<-read.table(file="import_datasets/aurelia_15minCell_statareas.txt",header=T,
               sep=",",stringsAsFactors = F)
head(d3)
library(tidyverse)
...
#Excel file
library(readxl)
e<-read_xlsx(path ="import_datasets/Aurelia_SEAMAP_2012-2018_30minCell.xlsx",
             sheet=1, col_names=T)
head(e)
